...
p.VaR<-0.99
p.ES<-0.975
t<-5/252 # lookback how many days
l<-5 
npaths <- 10000 # generate how many data per date to calculate Monte Carlo VaR and ES
bktesting.length <- 1 # Back testing length??是这个名字么，就是backtest1年数据这里就是1
...
MC.results2 <- matrix(NA,ncol = 2, nrow = VaR_length)

# Calculate the Monte VaR and ES
Type <- ifelse(data.option[2,4]>0,"p","c")
flag =( ncol(data.stock)-2)/2
for (i in 1:VaR_length) {
  MC.results2[i,] <- MC(investment = 10000,reducepct =0,current_date=i, 
                  stock=stock, stockposition=stockposition, rtn=stockrtn, 
                   iv=data.option[i,3]/100,mat=1,Type,r=0.005,optionshare=0,
                   p.VaR = 0.99, p.ES = 0.97, t = 5/252, l = 5, npaths = 10000,flag)
}


     # Get VaR and ES 
MC.VaR <- rev(MC.results2[,1])
MC.ES <- rev(MC.results2[,2])
...
########################################################################################################
temp.df<-data.frame(date_series,ParaVaR,HistVaR,ParaES,HistES,MC.VaR,MC.ES,realloss[VaR_length:1])
...
ggplot(temp.df)+
  geom_line(mapping=aes(x=Date,y=ParaVaR,color="Parametric VaR"))+
  geom_line(mapping=aes(x=Date,y=realloss[VaR_length:1],color="Actual Loss"))
